...
public class MartialController : MonoBehaviour
{
    ...
    void Update()
    {
        if (GetComponent<Character>().life <= 0)
        {
            ...
            martialRange.GetComponent<CircleCollider2D>().enabled = false;
            GetComponent<CapsuleCollider2D>().enabled = false;
            this.enabled = false;
        }

        if (skin.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("MartialAttack"))
        {
            return;
        }
